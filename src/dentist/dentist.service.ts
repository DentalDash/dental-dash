...
export class DentistService {
  ...
  async deleteDentist(croNumber: string) {
    const result = await this.dentistRepository.delete({
      croNumber: croNumber,
    });
  ...
  async findDentistById(id: string): Promise<Dentist> {
    const dentist = await this.dentistRepository.findOne({ where: { id: id } });

    if (!dentist) throw new NotFoundException('Dentista não encontrado');
    return dentist;
  }
  ...
  async updateDentist(
    ...
    const dentistToUpdate = await this.findDentistById(id);
