import { IsBoolean, IsEmpty, IsNotEmpty, Length, Matches } from "class-validator";
...
export class UpdateDentistDto {
    ...
    @IsNotEmpty(
        { message: 'O campo número CRO não pode estar vazio' }
    )
    ...
    @IsNotEmpty(
        { message: 'O campo estado CRO não pode estar vazio' }
    )
    
    croState: EstadoCRO;
    
    @IsBoolean(
        { message: 'O campo isAdmin deve ser um booleano' }
    )
